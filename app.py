...
import bcrypt
import time
import pandas as pd # Přidáváme podporu pro tabulky
...
if st.session_state.get("logged_in"):
    ...
    if volba == "Kalendář akcí":
        st.title(f"Přehled akcí: {st.session_state['user_sdh']}")
        c_form, c_list = st.columns([1, 3])
        ...
        with c_form:
            with st.form("nova"):
                ...
                if st.form_submit_button("Přidat událost"):
                    db.insert_akce({"sdh": st.session_state["user_sdh"], "typ_akce": typ, "nazev": nazev, "datum_jednorazove": datum.isoformat(), "cas": cas.strftime("%H:%M")})
                    st.rerun()
        ...
        with c_list:
            ...
            for a in akce:
                ...
                tabulka_data.append({
                    "Typ": a['typ_akce'],
                    "Název": a['nazev'],
                    "Datum": a.get('datum_jednorazove', ''),
                    "Čas": a.get('cas', ''),
                    "Účast (potvrzeno)": len(prihl),
                    "ID": a['id'] # Skryté ID pro tlačítka
                })
            
            df = pd.DataFrame(tabulka_data)
            
            if not df.empty:
                st.dataframe(df.drop(columns=["ID"]), use_container_width=True, hide_index=True)
                
                st.write("---")
                st.subheader("Správa docházky (vyberte akci)")
                vyber = st.selectbox("Akce pro zápis docházky:", options=akce, format_func=lambda x: f"{x['nazev']} ({x['datum_jednorazove']})")
                
                c1, c2, c3 = st.columns(3)
                if c1.button("Přijdu na vybranou akci"): db.uloz_dochazku(vyber['id'], st.session_state['user_id'], "Přijdu"); st.rerun()
                if c2.button("Nepřijdu na vybranou akci"): db.uloz_dochazku(vyber['id'], st.session_state['user_id'], "Nepřijdu"); st.rerun()
                if c3.button("Smazat vybranou akci"): db.delete_akce(vyber['id']); st.rerun()
            else:
                st.info("Zatím nejsou naplánovány žádné akce.")
